"""
【模块说明】HTTP 请求头与响应头管理（Headers）— 统一处理前后端通信的元数据

HTTP 请求头（Request Headers）是客户端发来的附加信息，比如"这个请求来自哪个平台"。
HTTP 响应头（Response Headers）是服务器返回给客户端的附加信息，比如跨域权限、缓存策略等。

这个模块把所有请求头的读取、校验，以及统一响应头的注入逻辑集中在一处，
避免在每个接口里重复写相同的处理逻辑。

【主要功能】
  RequestHeaders  — 从请求头中读取客户端类型、设备信息等，转换为 context 字典
  ResponseHeaders — 设置所有接口统一返回的响应头（如 CORS 跨域、缓存控制等）
