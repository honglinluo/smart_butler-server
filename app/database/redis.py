...
封装了对 Redis 内存数据库的所有操作。
Redis 是一个超快的内存数据库，适合存储需要频繁读写的临时数据。

【在本系统中的用途】
  - 登录会话（Token）存储：用户登录后的身份令牌
  - 最近对话记录：最近 10 轮对话快速加载，无需每次查数据库
  - 用户画像缓存：偏好设置等不常变的用户数据预加载
  - 定时任务通知队列：待推送给用户的消息
  - 对话记忆的临时锁：防止并发压缩操作冲突

【特点】
  数据存在内存中，读写速度比 MySQL 快 100 倍以上，
  但断电或重启后数据会丢失，所以只用于临时/可重建的数据。
"""
