"""
【模块说明】工具动态加载器（ToolLoader）— 让用户和 AI 能够"创建"新工具

内置工具是开发者提前写好的。但如果用户或 AI 想要一个当前不存在的工具，
这个模块提供了两条动态创建路径：

【用户创建工具的流程】
  1. 用户在前端提交工具代码（Python 函数）+ 工具说明
  2. 系统编译并验证代码（安全检查）
  3. 保存到数据库，注册到工具表
  4. 服务重启后自动从数据库恢复

【AI（Code Agent）创建工具的流程】
  1. Agent 在执行任务时发现需要一个新工具
  2. 发出 ToolCodeRequest 请求（通过事件循环）
  3. ToolLoader 编译、实例化，立即可用（不需要重启）
  4. 该工具只对创建它的 Agent 可见（exclusive 权限）

【安全限制】
  动态工具代码运行在受限命名空间中，禁止导入 os.system、subprocess 等危险模块
