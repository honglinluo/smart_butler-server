"""
【模块说明】工具创建示例（Examples）— 演示如何用不同方式创建工具

这个文件展示了三种创建工具的方式，以及各种权限/可见性配置：
  1. @tool 装饰器方式（推荐）— 声明式，最简洁
  2. 继承 BaseTool 方式    — 面向对象，适合复杂工具
  3. 函数式方式            — 最直接，适合简单功能

【可见性和权限分类】
  VIS_PUBLIC    — 所有用户和 Agent 都可使用
  VIS_EXCLUSIVE — 只有创建它的 Agent 可使用

【执行位置】
  EXEC_SERVER — 在服务器上执行（可访问数据库、文件系统等）
  EXEC_CLIENT — 在客户端浏览器执行（适合前端 UI 操作）
