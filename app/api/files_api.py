"""
【模块说明】文件管理 API — 让用户查看和下载自己的文件

用户上传的文件和 AI 生成的文件都保存在服务器上，这些接口让用户可以管理这些文件。

【可用接口】
  GET    /files/list             — 列出我的所有文件（可按类型筛选：上传的/AI生成的）
  GET    /files/download/{id}    — 下载指定文件
  DELETE /files/{id}             — 删除指定文件

【安全机制】
  每个文件有一个 file_id（加密的路径标识），下载时会验证这个文件确实属于当前用户，
  防止用户猜测 ID 下载别人的文件（越权访问攻击）。
