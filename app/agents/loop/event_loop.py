"""
【模块说明】Agent 事件循环（AgentEventLoop）— 驱动 Agent 完成复杂任务的执行引擎

普通对话 Agent 只能用已有的工具，但复杂任务可能需要它临时"造"新工具。
这个模块实现了一个多轮迭代循环：

【执行流程】
  第一轮  → 让 Agent 尝试完成任务
  发现    → Agent 回复中包含"需要新建工具"的信号（ToolCodeRequest JSON）
  构建工具 → 先问用户"可以建这个工具吗？"（决策门控）
             → 用户同意后，让另一个 AI（CodeAssistant）编写工具代码
             → 工具代码写入文件，注册到工具表
  再次尝试 → 告诉 Agent "工具已就绪"，重新调用它
  重复    → 直到任务完成或达到最大迭代次数

【用户决策门控（三种策略）】
  allow → 自动放行，不打断用户（适合受信任的 Agent）
  ask   → 每次构建工具前暂停，推送弹窗让用户确认（默认）
  deny  → 拒绝所有工具构建请求
