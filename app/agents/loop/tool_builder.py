"""
【模块说明】动态工具构建器（ToolBuilder）— 让 AI 按需"造"新工具

当 Agent 遇到自己没有合适工具的任务时，可以请求构建一个专用工具。
这个模块负责调用 AI（CodeAssistant）生成工具代码，验证语法，然后写入文件。

【工具生成规范】
  - 函数命名：tool_{工具名}
  - 返回格式：{"success": bool, "result": 任意结果, "error": 错误信息}
  - 只能使用 Python 标准库（不允许引入外部包，避免安全风险）
  - 生成后做 AST 语法检查，不合法则拒绝写入

【生成的工具存放位置】
  app/tools/dynamic/ 目录下，文件名为 {工具名}.py
