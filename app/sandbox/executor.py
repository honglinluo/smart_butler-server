"""
【模块说明】沙箱执行器（Sandbox Executor）— 安全地运行用户提交的代码

当 AI 或用户提交一段代码，需要"在服务器上运行"时，直接运行是非常危险的——
恶意代码可能删除文件、访问数据库、向外发送请求等。这个模块提供了一个"沙箱"：
在严格限制的隔离环境中运行代码，确保安全。

【安全措施】
  1. 独立临时目录：每次执行分配一个新目录，执行完自动清理
  2. 超时限制：超过 10 秒自动终止，防止无限循环
  3. 内存/CPU 限制：Linux 平台限制资源使用上限
  4. 阻断出向网络：清除代理环境变量，代码无法向外发请求
  5. 静态扫描拦截：执行前检查代码是否包含已知危险调用（如 os.system、subprocess 等）

【支持的执行方式】
  Python 代码   → 真正执行，返回输出结果
  Shell 脚本    → 仅做语法检查（不实际运行）
  JavaScript    → 仅做语法检查（不实际运行）
  其他语言/文本 → 仅做文本安全扫描，不执行
