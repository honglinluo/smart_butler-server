"""
【模块说明】内容扫描器（Scanner）— 检查用户上传的内容是否安全

在允许用户上传文件或提交文本之前，系统需要先"检查"一遍：
这是什么类型的文件？里面有没有危险内容？这个模块就是这道安全关卡。

【三项主要检查】
  1. 文件类型识别
     先看文件头部的"魔术字节"（文件内容的前几个字节），而不只是靠文件扩展名。
     这样可以防止有人把 EXE 文件改名为 .txt 来绕过检查。

  2. 代码片段提取
     从长文本（如聊天消息）中找出代码块（```python ... ``` 这类 Markdown 格式），
     方便后续交给沙箱执行或做安全扫描。

  3. 图片安全检测
     检测图片是否是"双重文件"（polyglot）——
     即一个文件同时是合法图片又是可执行代码（一种少见但危险的攻击方式）。

【恶意模式扫描】
  对文本内容做关键词扫描，拦截高危 shell 命令、系统调用等已知危险模式。
