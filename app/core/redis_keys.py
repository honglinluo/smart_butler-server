"""
【模块说明】Redis 缓存键名规范 — 系统所有缓存键的统一定义

Redis 是一个内存数据库，用于存储需要快速读写的临时数据。
本文件定义了系统中所有 Redis 键名（Key）的格式，统一在此管理，
方便维护和避免各模块使用不一致的键名。

【主要缓存类别】
  - 登录会话（session:token:...）      — 记录哪个 Token 对应哪个用户
  - 用户初始化数据（user:...:init）    — 用户画像等登录后预加载的数据
  - 对话记忆（memory:...:turns 等）    — 最近对话轮次、同步锁、压缩状态
  - 通知队列（notify:...:pending）     — 待推送给用户的通知消息
  - 背景预取（memory:...:prefetch）    — 下一轮对话用到的记忆预热缓存
  - 定时任务相关、Agent 执行日志等
