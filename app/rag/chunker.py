"""
【模块说明】对话切片器（Chunker）— 把一轮对话拆成多个可检索的小片段

做向量搜索时，如果把整段很长的对话存成一个向量，检索精度会很差。
更好的做法是把一轮对话拆成多个小片段（chunk），每段单独生成向量。
这样搜索时可以精确定位到"用户问的哪句话"或"AI 回答的哪部分"。

【Chunk 的三种类型】
  question     — 用户提问的文本片段
  answer       — AI 回复的文本片段
  agent_output — 某个 Agent 输出的文本片段

【Chunk ID 格式】
  {轮次ID}_q0, {轮次ID}_q1 ...  → 用户问题的第1、2个片段
  {轮次ID}_a0, {轮次ID}_a1 ...  → AI 回复的第1、2个片段
