"""
【模块说明】混合检索器（Hybrid Retriever）— 用多种方式找到最相关的历史记忆

当 AI 要回答用户问题时，需要先在历史记忆中搜索相关内容。
单一搜索方式可能有遗漏，所以这里用了三种互补的方式组合搜索：

【三级检索策略（优先级从高到低）】
  1. 预取缓存（Redis）
     上轮对话结束时已提前算好，直接从缓存取，最快（0延迟）

  2. 向量语义检索（Elasticsearch KNN）
     把用户问题转成向量，找"语义上接近"的历史内容
     优点：能找到表达不同但意思相近的内容
     过滤：余弦相似度必须超过 0.7 才保留

  3. 全文关键词检索（Elasticsearch BM25）
     传统的关键词匹配，作为兜底或补充
     过滤：得分必须达到最高分的一定比例

【同会话优先】
  来自同一客户端（如飞书/微信）本次会话的相关记忆会获得额外加分，
  让近期上下文更容易出现在 AI 的参考信息中。
