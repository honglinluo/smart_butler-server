"""
【模块说明】记忆格式化器（Formatter）— 把检索到的记忆整理成 AI 可读的格式

从历史中检索到的记忆片段，不能直接扔给 AI，需要先做格式化处理：
  1. 用 <memory-context>...</memory-context> 标签把记忆包裹起来
     这样 AI 就知道"这段内容是背景参考信息，不是用户当前说的话"
  2. 防止重复嵌套：如果记忆内容本身已有这个标签，先清除再重新包裹
  3. 附加时间戳和来源信息，帮助 AI 判断记忆的时效性
